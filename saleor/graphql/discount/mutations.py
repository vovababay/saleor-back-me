...
from .enums import DiscountValueTypeEnum, VoucherTypeEnum, FrequencyValueTypeEnum
from .types import Sale, Voucher
from ...discount.tasks import update_value_discount
...
class SaleInput(graphene.InputObjectType):
    ...
    periodically_change = graphene.Boolean(require=True) # Статус менять ли скидку динамически
...
def start_period(instance):
    tomorrow = datetime.now() + timedelta(minutes=1)
    update_value_discount.apply_async(kwargs={"discount_pk": instance.pk}, eta=tomorrow)
...
class SaleUpdateMinimalVariantPriceMixin:
    ...
    def success_response(cls, instance):
        ...
        #print(f"instance:  {instance.value}     {instance.start_date}    {instance.end_date}   {instance.periodically_change}")
        if instance.periodically_change:
            start_period(instance)
